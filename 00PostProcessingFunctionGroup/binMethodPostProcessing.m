...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function binMethodPostProcessing(sample_R,sample_TH,sample_Z,num_of_z_mesh)
...
global velocityPostprocess velocity_RMS_PDF_Skewness outputStreaks spaceSpectraAnalysis calUcTauWall lambda2Criterion QCriterion outputStreaksAll wallShearStressStreaksAll
global pressurePostprocess pressure_RMS_PDF_skewness SpaceTimeDiagram corrContour multiMode pressure_waveSpectra wallShearStressStreaks

%% some confinement
if ((velocityPostprocess == 0) && ((velocity_RMS_PDF_Skewness + spaceSpectraAnalysis) > 0))
...
[DelaunayMatrix,weightCoeff,new_coord]      = DelaunayScatterInterpN(coord,new_coord,file_location,num_of_z_mesh);
...
[uz,ur,uth]                = ofVelocityReadingFunction(folder,numOfCells,theta,file_location,DelaunayMatrix,weightCoeff,extended_indx);
end
...
if(outputStreaksAll == 1)
    velocityStreakDataOutputAllTimeSlots(ur,uth,uz,avg_R,avg_th,avg_z,loc_r_group,loc_th_group,loc_z_group,temp_dir);
end

% find the wall-shear stress streaks
if(wallShearStressStreaks == 1)
    wallShearStressStreaksOutput(ur,uth,uz,avg_R,avg_th,avg_z,loc_r_group,loc_th_group,loc_z_group,temp_dir);
end

if(wallShearStressStreaksAll == 1)
    wallShearStressStreaksOutputAllTimeSlots(ur,uth,uz,avg_R,avg_th,avg_z,loc_r_group,loc_th_group,loc_z_group,temp_dir);
end
...
if(pressure_waveSpectra == 1)
pressureWaveNumber1DSpatialSpectra(avg_R,avg_th,avg_z,loc_r_group,loc_th_group,p,p_sptime_avg,uz_sptime_avg,1,temp_dir);
end
