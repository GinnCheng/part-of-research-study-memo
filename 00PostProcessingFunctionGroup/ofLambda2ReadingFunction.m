%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% Function to read lambda2 files
 %  code by ginn
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function lambda2 = ofLambda2ReadingFunction(t,numOfCells,file_location,DelaunayMatrix,weightCoeff,extended_indx)
%   convert ux uy to ur utheta and record uz for all time steps
lambda2     = zeros(numOfCells, length(t));
%   record ur utheta and uz for each time step
for k = 1:length(t)
    disp(['now reading lambda2 time = ',num2str(t(k),'%.16g')]);
    % import velocity data for each time step
    temp_location = [file_location,'/%.16g/%s'];
    filename      = sprintf(temp_location,t(k),'Lambda2');
    ...
    frewind(fid);
    % now read the data
    templambda2   = textscan(fid, '%f','HeaderLines',field_indx+2);
    fclose(fid);
    % Instantaneous lambda2
    temp          = templambda2{1}(extended_indx);
    triVals       = temp(DelaunayMatrix);
    lambda2(:,k)  = weightCoeff.*triVals*[1;1;1;1];
end
    clear templambda2 temp;
end